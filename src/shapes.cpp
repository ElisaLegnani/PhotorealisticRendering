...
Vec2d _sphere_point_to_uv(Point sp) {
  ...
  if(u>=0.0){ return Vec2d(u,v);}
  else{return Vec2d((u+1.0),v);}
}

Normal _sphere_normal(Point sp, Vec ray_dir){
  Normal normal=Normal(sp.x, sp.y,sp.z);
  if(dot(sp.to_vec(),ray_dir)<0.0){return normal;}
  else{return (-normal);}
}
...
HitRecord Sphere::ray_intersection(Ray ray){
    ...
    float first_hit_t;
    if (tmin > inv_ray.tmin && tmin < inv_ray.tmax){first_hit_t = tmin;}
    else if(tmax > inv_ray.tmin && tmax < inv_ray.tmax){first_hit_t = tmax;}
    else{ return HitRecord();}

    Point hit_point = inv_ray.at(first_hit_t);
            
  return HitRecord((transformation * hit_point),(transformation * _sphere_normal(hit_point, inv_ray.dir)),(_sphere_point_to_uv(hit_point)),first_hit_t,ray);
